...
public class TestDownloadCSR extends TestCase {
  ...
  public void testDownloadLocalCSRSuperAdmin() throws Exception {
    ...
    assertEquals(
        "The GET request should succeed. Getting status code "
            + httpResponse.getStatusLine().getStatusCode(),
        HttpStatus.SC_OK,
        httpResponse.getStatusLine().getStatusCode());
    String downloadedContent =
        new String(ByteUtil.getContent(httpResponse.getEntity().getContent(), -1));
    assertNotNull("downloaded empty content", downloadedContent);
    assertTrue(
        "downloaded content is not the same as CSR file on disk",
        downloadedContent.equals(csrContent));
  }
  ...
  public void testDownloadCSRDelegatedAdmin() throws Exception {
    ...
    HttpResponse httpResponse = HttpClientUtil.executeMethod(httpClientBuilder.build(), get);
    assertEquals(
        "The GET request should succeed. Getting status code " + httpResponse,
        HttpStatus.SC_OK,
        httpResponse);
    String downloadedContent =
        new String(ByteUtil.getContent(httpResponse.getEntity().getContent(), -1));
  ...
  String getDownloadURL() throws Exception {
    ...
    String downloadURL =
        "https://"
            ...
            + port
            + "/service/extension/"
            + ZimbraCertMgrExt.EXTENSION_NAME_CERTMGR
            + "/"
            + DownloadCSRHandler.HANDLER_PATH_NAME;
    return downloadURL;
  }
